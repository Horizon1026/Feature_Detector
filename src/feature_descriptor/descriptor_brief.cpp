...
bool BriefDescriptor::ComputeForOneFeature(const GrayImage &image,
    ...
    for (int32_t i = 0; i < options().kLength; ++i) {
        ...
        const Vec2 point_2 = rot * Vec2(pattern_idx_[idx_4 + 2], pattern_idx_[idx_4 + 3]) + pixel_uv;
        const float value_1 = image.GetPixelValueNoCheck(point_1.y(), point_1.x());
        const float value_2 = image.GetPixelValueNoCheck(point_2.y(), point_2.x());
        if (value_1 < value_2) {
            descriptor[i] = 1;
        }
    }
...
std::array<int16_t, 256 * 4> BriefDescriptor::pattern_idx_ = {
