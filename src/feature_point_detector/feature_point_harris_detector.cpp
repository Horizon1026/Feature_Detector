...
bool FeaturePointHarrisDetector::ComputeGradient(const GrayImage &image) {
...
float FeaturePointHarrisDetector::ComputeResponseOfPixel(const GrayImage &image, const int32_t row, const int32_t col) {
    Mat2 M = Mat2::Zero();
    int32_t cnt = 0;
    ...
    return det - feature_options_.kAlpha * trace * trace;
}
