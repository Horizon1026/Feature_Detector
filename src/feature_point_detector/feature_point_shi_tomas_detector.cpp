...
void FeaturePointShiTomasDetector::ComputeHorizontalGradientSums(const GrayImage &image) {
...
void FeaturePointShiTomasDetector::ComputeResponseMap() {
    ...
    for (int32_t r = bound; r < rows - bound; ++r) {
        ...
        for (int32_t c = bound; c < cols - bound; ++c) {
            if (this->mask()(r, c)) {
                const float a = sum_xx_vec[c] * inv_cnt;
                const float c_val = sum_yy_vec[c] * inv_cnt;
                if (a + c_val > options().kMinValidResponse) {
                    const float b = sum_xy_vec[c] * inv_cnt;
                    const float diff = a - c_val;
                    const float common = std::sqrt(diff * diff + 4.0f * b * b);
                    const float res = (a + c_val + common) * 0.5f;
...
void FeaturePointShiTomasDetector::PerformNMSAndExtractCandidates() {
